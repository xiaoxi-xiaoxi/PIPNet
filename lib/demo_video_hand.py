...
from hand_detector.detector import Detector
from hand_detector.tools import handle_preds
...
model = Detector(1, True).to(device)
weight = r'C:\Users\wangyuan\Desktop\demo\detector\weight_AP05_0.840290_200-epoch.pth'
...
def demo_video(video_file, net, preprocess, input_size, net_stride, num_nb, use_gpu, device):
    ...
    while (cap.isOpened()):
        ...
        if ret == True:
            ...
            res_img = cv2.resize(frame, (256, 256), interpolation=cv2.INTER_LINEAR)
            img = res_img.reshape(1, 256, 256, 3)
            ...
            scale_h, scale_w = frame_height / 256, frame_width / 256

            # 特征图后处理
            output = handle_preds(preds, device, 0.8)
            for i,box in enumerate(output[0]):
                ...
                det_xmin, det_ymin = int(box[0] * frame_width), int(box[1] * frame_height)
                det_xmax, det_ymax = int(box[2] * frame_width), int(box[3] * frame_height)
