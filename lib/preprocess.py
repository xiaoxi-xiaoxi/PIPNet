...
# hand!!!!
def process_DO(root_folder, folder_name, image_name, label_name, target_size):
    image_path = os.path.join(root_folder, folder_name, image_name)
    label_path = os.path.join(root_folder, folder_name, label_name)
...
def process_HaGRID_mini(image_path, img_anno, target_size):
    image = cv2.imread(image_path)
    image_height, image_width, _ = image.shape
    ...
    for i in range(len(landmarks)):
        ...
        #
        # draw_kp = np.array([anno_x, anno_y]).transpose()
        # colors = [(0, 215, 255), (255, 115, 55), (5, 255, 55), (25, 15, 255), (225, 15, 55)]
        # cv2.circle(image, (draw_kp[0]), 8, (255, 0, 0), 4)
        # for i in range(1, len(draw_kp)):
        #     cv2.circle(image, (draw_kp[i]), 8, colors[(i - 1) // 4], 4)
        #
        ...
        # x2_1 = min(image_width, cx + int(ratio * length))
        # y2_1 = min(image_height, cy + int(ratio * length))

        # scale = 1.25
        # scale = random.random()*1.25+1
        # bbox_xmin -= int((scale - 1) / 2 * bbox_width)
        # bbox_ymin -= int((scale - 1) / 2 * bbox_height)
        # bbox_width *= scale
        # bbox_height *= scale
...
def gen_data(root_folder, data_name, target_size):
    ...
    if data_name == 'data_300W':
        ...
        for folder_train in folders_train:
            ...
            for image_name, label_name in zip(image_files, label_files):
                print(image_name)
                image_crop, anno = process_300w(os.path.join(root_folder, 'data_300W'), folder_train, image_name,
        ...
        for folder_test in folders_test:
            ...
            for image_name, label_name in zip(image_files, label_files):
                print(image_name)
                image_crop, anno = process_300w(os.path.join(root_folder, 'data_300W'), folder_test, image_name,
                                                label_name, target_size)
    ...
    ## hand!!!!
        ...
        folders_train = ['Grasp1', 'Grasp2', 'Occlusion']
        annos_train = {}
        for folder_train in folders_train:
            all_files = sorted(os.listdir(os.path.join(root_folder, data_name, folder_train)))
            image_files = [x for x in all_files if '.pts' not in x]
    ...
    elif data_name == 'HaGRID_mini':
        folders = os.listdir(os.path.join(root_folder, data_name, 'subsample'))
        random.shuffle(folders)
        folders_train = folders[:17]

        annos_train = {}
        for folder_train in folders_train:
            all_files = os.path.join(root_folder, data_name, 'subsample', folder_train)
            image_files = os.listdir(all_files)
            label_file = all_files.replace('subsample', 'ann_subsample') + '.json'
            with open(label_file) as f:
                label_anno = json.load(f)
            for image_name in image_files:
                print(image_name)
                if not image_name.endswith('.jpg'):
                    continue
                image_path = os.path.join(all_files, image_name)
                image_name_only = image_name.split('.')[0]
                img_anno = label_anno[image_name_only]
                image_crops, annos = process_HaGRID_mini(image_path, img_anno, target_size)
                if not image_crops:
                    continue
                if len(image_crops) == 1:
                    image_crop_name = folder_train + '_' + image_name
                    image_crop = np.ascontiguousarray(image_crops[0])
                    cv2.imwrite(os.path.join(root_folder, data_name, 'images_train', image_crop_name), image_crop)
        ...
        folders_test = folders[17:]
        annos_test = {}
        for folder_test in folders_test:
            all_files = os.path.join(root_folder, data_name, 'subsample', folder_test)
            image_files = os.listdir(all_files)
            label_file = all_files.replace('subsample', 'ann_subsample') + '.json'
            with open(label_file) as f:
                label_anno = json.load(f)
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        gen_data(r'C:\Users\wangyuan\Desktop\program\dataset\gesture', data_name, 256)
